...
public class CoreLogic
{
    ...
    public static void CoreLogicMain(string[] args)
    {
        ...
        PrintFolders.PrintToStream(BookmarkImporterFactory.SmartImport("test1.html"));
        Console.WriteLine("\n\n\n");
        PrintFolders.PrintToStream(BookmarkImporterFactory.SmartImport("test2.html"));
        Console.ReadKey();
        Environment.Exit(0);
#endif
        ...
        List<ImportedFolder> importedFolders = new List<ImportedFolder>();
        List<MappedFolder> mappedFolders = new List<MappedFolder>();
        List<ResolvedFolder> resolvedFolders = new List<ResolvedFolder>();
        ...
        if (setOptions.Interactive)
        {
            ...
            if (!string.IsNullOrEmpty(filePath))
            {
                ...
                importedFolders = BookmarkImporterFactory.SmartImport(filePath);
            }
            else
            {
            ...
            mappedFolders = FolderManager.CreateFolderStructure(importedFolders, rootdir);
            resolvedFolders = AutoImport.LinksFromUrls(mappedFolders);
            AutoImport.WriteLinksToTextFiles(resolvedFolders, downloadPlaylists, downloadChannels, downloadShorts,
                rootdir);

            PrintFolders.PrintToStream(resolvedFolders); //dump all the folder info to console
            AutoImport.Scriptwriter(resolvedFolders, ytdlp_path); //writing the scripts that call yt-dlp and add .txt with the links in the arguments //NOT the method that creates the .txt files
            FolderManager.DeleteEmptyFolders(mappedFolders); //deletes the folders from the folder structure that are empty (no youtube links were written into them)
            Console.WriteLine("Running the scripts after ENTER.");
            Console.ReadKey();
            AutoImport.Runningthescripts(resolvedFolders);
            //AppMethods.Checkformissing //checking if all the desired links have indeed been downloaded, archive.txt integrity as well
            PrintFolders.PrintToStream(resolvedFolders);
        ...
        else //non-interactive
        {
            ...
            mappedFolders = FolderManager.CreateFolderStructure(importedFolders, rootdir);
            resolvedFolders = AutoImport.LinksFromUrls(mappedFolders);
            AutoImport.WriteLinksToTextFiles(resolvedFolders, downloadPlaylists, downloadChannels, downloadShorts, rootdir);
